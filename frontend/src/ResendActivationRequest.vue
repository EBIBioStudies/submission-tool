      ...
      <div class="card">
        <div class="card-body">
          <div v-if="success">
            ...
            <h6 class="card-subtitle mb-2 text-muted">The activation link has been sent to <b>{{ email }}</b>. Please
              check your
              email, including the spam/trash folder.</h6>
          </div>
          <div v-else>
            <h4 class="card-title">Send activation email</h4>
            <h6 class="card-subtitle mb-2 text-muted">Please enter your email and we will send you a link to activate
              your
              account.</h6>
            <form class="" data-testid="resetReqForm" name="pwdReqForm" @submit.prevent="submitData">
              <div class="form-group">
                <label for="email">Email</label> <input id="email" v-model="email"
                                                        :class="{'is-invalid': email && !validEmail}" autofocus
                                                        class="form-control"
...
const submitData = async () => {
  if (validEmail.value && validCaptcha.value) {
    ...
    try {
      await axios.post(`/api/auth/retryact`, parameters);
      success.value = true;
    } catch (error: any) {
