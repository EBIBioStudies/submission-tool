    ...
    <div class="col-6 gy-6">
      <div class="card">
        <div v-if="success" class="card-body">
          ...
          <h6 class="card-subtitle mb-2 text-muted">An email has been sent with a link to activate your new
            account.</h6>
        </div>
        <div v-if="!success" class="card-body">
          ...
          <form @submit.prevent="submit" name="signUpForm" data-testid="signUpForm">
            <div class="form-group mb-2">
              ...
              <input type="text" id="name" v-model="form.name" required class="form-control"
                     :class="{'is-invalid': form.name && !validName}">
            ...
            <div class="form-group">
              ...
              <input type="email" id="email" v-model="form.email" required class="form-control"
                     :class="{'is-invalid': form.email && !validEmail}">
            ...
            <div class="form-group">
              ...
              <input type="password" id="password" v-model="form.password" minlength="6" required class="form-control"
                     :class="{'is-invalid': form.password && !validPassword}">
              <div v-if="form.password && !validPassword" class="invalid-feedback">Password minimum length is 6
                characters
              </div>
            </div>
            <div class="form-group">
              ...
              <input type="text" id="orcid" placeholder="1111-2222-3333-4444" v-model="form.orcid"
                     pattern="\d{4}-\d{4}-\d{4}-\d{3}[0-9X]" class="form-control" :class="{'is-invalid': !validOrcid}">
              <div v-if="!validOrcid" class="invalid-feedback">Please enter a valid orcid in a 4*4 format, e.g.,
                0000-0001-2345-6789
              </div>
            </div>
            <div class="form-check">
              <input type="checkbox" id="terms" v-model="form.terms" required class="form-check-input">
              <label for="terms"> I have read and agree to the <a target="_blank"
                                                                  href="https://www.ebi.ac.uk/data-protection/privacy-notice/embl-ebi-public-website/">Privacy
                Notice</a> and <a target="_blank" href="https://www.ebi.ac.uk/about/terms-of-use">Terms of Use</a>,
                including the limited processing of personal data.</label>
            </div>
            <button type="submit" :class="{disabled: !formValid}" class="btn btn-primary"> Register</button>
            <vue-recaptcha v-if="!isCaptchaVerified" required="" class="captcha-root" :sitekey="captchaPublicKey"
                           @verify="onCaptchaVerified"></vue-recaptcha>
          </form>
        </div>
