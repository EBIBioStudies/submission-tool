    ...
    <div class="card col-6 gy-6">
      <div class="card-body">
        <div v-if="success">
          ...
          <h6 class="card-subtitle mb-2 text-muted">The activation link has been sent to {{email}}. Please check your email, including the spam/trash folder.</h6>
        </div>
        <div v-else>
          <h4 class="card-title">Send activation email</h4>
          <h6 class="card-subtitle mb-2 text-muted">Please enter your email and we will send you a link to activate your account.</h6>
          <form @submit.prevent="submitData" name="pwdReqForm" data-testid="resetReqForm" class="">
            ...
            <button type="submit" class="btn btn-primary my-2">Send activation link</button>
