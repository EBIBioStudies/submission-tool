...
import type { Node } from '../models/FileTreeNode.model';
...
onMounted(show);
export interface FileTreeExpose {
  show: () => void;
}
defineExpose<FileTreeExpose>({ show });


const onSelected = (node: Node) => {
...
const onChildSelected = (selectedChild: Node) => emit('select', selectedChild);
