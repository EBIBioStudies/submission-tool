...
<template>
  ...
  <div v-if="canRender(props.section)" class="section-block pb-2" :class="{ collapsed: isCollapsed }">
    <div v-if="section?.type === 'Study'" :key="sectionsRefreshKey">
      <Authors v-if="section" :section="section"
        :sectionType="sectionType?.tableTypes?.find((s) => s.name.toLowerCase() === 'contact')" ref="authorComponent" />
    </div>
    <!-- section title -->
    <div class="align-text-bottom">
      <span class="col" :class="[depth > 0 ? 'branch' : 'branch spacer']"></span>
      <span class="input-group-text text-start btn btn-lg ps-1 mt-2 section-title" @click="isCollapsed = !isCollapsed">
        <font-awesome-icon class="section-control"
          :icon="'fa-caret-' + (isCollapsed ? 'right' : 'down')"></font-awesome-icon>
        <span v-if="typeof props.sectionType !== 'string' || subSectionTypeMap.has(props.sectionType?.toLowerCase())"
          class="ms-2" :data-bs-toggle="sectionType?.description ? 'tooltip' : false" data-bs-html="true"
          :data-bs-title="sectionType?.description"><font-awesome-icon v-if="props.sectionType?.icon" class="icon"
            :icon="props.sectionType?.icon" />{{ section.type }}</span>
        <span v-else>
          <EditableLabel class="ms-2" @click.stop="" type="text" placeholder="Enter section type" :is-editable="true"
            :data="thisSection" />
        </span>
      </span>
      <span
        v-if="(sectionType?.display !== 'required' || thisSection?.accno?.includes('-removable')) && parentDisplayType !== 'readonly' && sectionType?.name !== 'Study' && !isPublicSubmission?.value"
        class="mt-2 btn btn-sm btn-delete" role="button">
        <font-awesome-icon role="button" @click="$emit('delete')" @click.stop="" icon="fa-trash"></font-awesome-icon>
      </span>
      <span v-if="depth === 0" class="float-end text-secondary" style="font-size: 8pt; padding-top:35px">*
        Required</span>
    </div>
    <!-- section content -->
    <transition name="slide">
      <div :class="{ 'visually-hidden': isCollapsed }">
        <div class="has-child-section ms-3 slide-in">
          ...
          <!-- attributes -->
          <Attributes :key="attributesRefreshKey" ref="attributesComponent" :attributes="section.attributes"
            ...
            @deleteAttribute="deleteAttribute" @createTag="createTag" @deleteTag="deleteTag"
            @newAttribute="addAttribute" />

          <div class="p-0" :key="sectionsRefreshKey">
            <!-- Files -->
            <SectionTable v-if="section.files && section.files?.length > 0" :rows="section.files"
              :depth="props.depth + 1" :sectionType="subSectionTypeMap.get('file')" sectionSubType="File"
              @rowsReordered="(e) => rowsReordered(e, section.files)"
              @columnUpdated="(msg) => updateColumnName(section.files, msg)"
              @columnsReordered="(msg) => sectionsRefreshKey += 1" @delete="deleteSubSection(section.files, 0)"
              @refreshSection="refreshSection()" ref="sectionFilesRef" />

            <!-- Links -->
            <SectionTable v-if="section.links && section.links?.length > 0" :rows="section.links"
              :depth="props.depth + 1" :sectionType="subSectionTypeMap.get('link')" sectionSubType="Link"
              @rowsReordered="(e) => rowsReordered(e, section.links)"
              @columnUpdated="(msg) => updateColumnName(section.links, msg)"
              @columnsReordered="(msg) => sectionsRefreshKey += 1" @delete="deleteSubSection(section.links, 0)"
              @refreshSection="refreshSection()" ref="sectionLinksRef" />
            ...
            <div v-for="(item, index) in specialSectionMap" :key="index" ref="sectionTablesRef">
              <SubSectionTable v-if="Array.isArray(item[1]) && item[1]?.length > 0" :rows="item[1]"
                :depth="props.depth + 1" :sectionType="subSectionTypeMap.get(item[0].toLowerCase())"
                :sectionSubType="item[0]" :parent=section @rowsReordered="(e) => rowsReordered(e, item[1])"
                @columnUpdated="(msg) => updateColumnName(item[1], msg)"
                @columnsReordered="(msg) => sectionsRefreshKey += 1"
                @delete="deleteSubSection(section.subsections, index)" @refreshSection="refreshSection()"
            ...
            <div v-for="(subsection, i) in section.subsections" :key="i" ref="subsectionsRef">
              ...
              <Section
                ...
                :depth="props.depth + 1" :sectionTypeMap="subSectionTypeMap"
                @delete="deleteSubSection(section.subsections, i)"
                @addTable="(msg) => addTable(msg.section, msg.instance)" ref="errSecRefs" />

              <!-- general Table -->
              <SectionTable v-if="Array.isArray(subsection) && subsection?.length > 0" :rows="subsection"
                :depth="props.depth + 1" :sectionType="subSectionTypeMap.get(subsection?.name?.toLowerCase())"
                :sectionSubType="subSectionTypeMap.get(subsection?.name?.toLowerCase())?.name" :parent=section
                @rowsReordered="(e) => rowsReordered(e, subsection)"
                @columnUpdated="(msg) => updateColumnName(subsection, msg)"
                @columnsReordered="(msg) => sectionsRefreshKey += 1" @delete="deleteSubSection(section.subsections, i)"
                @refreshSection="refreshSection()" ref="errSecTableRefs" />
            </div>
            <SubsectionMenu :sectionType="sectionType" @newSection="(type) => addSubsection(section, 1, type)"
              @newTable="(type) => addTable(section, 1, type)"></SubsectionMenu>
